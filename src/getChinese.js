...
function getFileList(dir, filesList = [], ignoredir) {
    ...
    files.forEach((item, index) => {
        ...
        if (stat.isDirectory()) {
            return getFileList(fullPath, filesList, ignoredir); // 递归读取文件
        } else {
...
function readFileList(ignoredir) {
    ...
    filesList.forEach((fullPath) => {
        ...
        if (['.vue', '.js', '.json', '.php'].includes(extname)) {
